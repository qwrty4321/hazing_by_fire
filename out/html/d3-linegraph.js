...
d3.linegraph = function(noTicks, noDots, parties, partyColors, partyNames, dataMax, dataMin, additionalMonths) {
    /* params */
    if (!parties) {
        parties = ['spd', 'uspd', 'fvp', 'z', 'nlp', 'dnvp', 'nsdap', 'other'];
    }
    if (!partyColors) {
        partyColors = {'spd': '#E3000F', 'uspd': '#700000', 'fvp': '#D3C24D', 'z': '#000', 'nlp': '#D5AC27', 'dnvp': '#3E88B3', 'nsdap': '#7A3C00', 'other': '#a0a0a0'};
    }
    if (!partyNames) {
        partyNames = {'spd': 'SPD', 'uspd': 'USPD', 'fvp': 'FVP', 'z': 'Z + BVP', 'nlp': 'NLP', 'dnvp': 'DNVP', 'nsdap': 'NSDAP', 'other': 'Others'};
    }
    if (!additionalMonths) {
        additionalMonths = 10;
    }
